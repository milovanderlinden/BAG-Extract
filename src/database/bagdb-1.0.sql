...
CREATE VIEW verblijfsobjectactueel AS
    SELECT (verblijfsobject.oid)::character varying AS oid, verblijfsobject.identificatie, verblijfsobject.aanduidingrecordinactief, verblijfsobject.aanduidingrecordcorrectie, verblijfsobject.officieel, verblijfsobject.inonderzoek, verblijfsobject.documentnummer, verblijfsobject.documentdatum, verblijfsobject.hoofdadres, verblijfsobject.verblijfsobjectstatus, verblijfsobject.oppervlakteverblijfsobject, verblijfsobject.verblijfsobjectgeometrie, verblijfsobject.begindatum, verblijfsobject.einddatum, verblijfsobject.geometrie FROM verblijfsobject WHERE (((verblijfsobject.begindatum <= ('now'::text)::date) AND (verblijfsobject.einddatum >= ('now'::text)::date)) AND ((verblijfsobject.aanduidingrecordinactief)::text = 'N'::text));
...
CREATE UNIQUE INDEX ligplaatskey ON ligplaats USING btree (identificatie, aanduidingrecordinactief, aanduidingrecordcorrectie, begindatum);
CREATE UNIQUE INDEX ligplaatsoid ON ligplaats USING btree (oid);
CREATE UNIQUE INDEX nummeraanduidingkey ON nummeraanduiding USING btree (identificatie, aanduidingrecordinactief, aanduidingrecordcorrectie, begindatum);
CREATE INDEX nummeraanduidingpostcode ON nummeraanduiding USING btree (postcode);
CREATE UNIQUE INDEX openbareruimtekey ON openbareruimte USING btree (identificatie, aanduidingrecordinactief, aanduidingrecordcorrectie, begindatum);
...
CREATE UNIQUE INDEX pandkey ON pand USING btree (identificatie, aanduidingrecordinactief, aanduidingrecordcorrectie, begindatum);
CREATE UNIQUE INDEX pandoid ON pand USING btree (oid);
CREATE INDEX standplaatsgist ON standplaats USING gist (geometrie);
CREATE UNIQUE INDEX standplaatskey ON standplaats USING btree (identificatie, aanduidingrecordinactief, aanduidingrecordcorrectie, begindatum);
...
CREATE UNIQUE INDEX verblijfsobjectkey ON verblijfsobject USING btree (identificatie, aanduidingrecordinactief, aanduidingrecordcorrectie, begindatum);
