...
class BAGZoekOpIdentificatie(BAGZoekPanel):
    def __init__(self, raadpleegScherm):
        ...
        self.zoekKnop = wx.Button(    self.cp.GetPane(), label="Zoek", pos=(120,120), size=(40,20))
    ...
    def zoek(self, event=None):
        ...
        logScherm("")
        obj = getBAGobjectBijIdentificatie(self.idText.GetValue())
        if not obj or not obj.leesActueelVoorkomenUitDatabase():
            logScherm("Geen object gevonden bij deze gegevens")
        else:
...
class BAGZoekOpPostcode(BAGZoekPanel):
    ...
    def zoek(self, event=None):
        ...
        if len(nummeraanduidingen) == 0:
            logScherm("Geen nummeraanduiding gevonden bij deze gegevens")
        else:
...
class BAGZoekOpCoordinaten(BAGZoekPanel):
    ...
    def zoek(self, event=None):
        ...
        if not x.isdigit() or not y.isdigit():
            logScherm("Ongeldige coordinaten")
            return
        ...
        logScherm("Geen objecten gevonden op deze coordinaten")
...
class BAGBoom(wx.TreeCtrl):
    ...
    def __init__(self, raadpleegScherm, positie, afmeting):
        wx.TreeCtrl.__init__(self, raadpleegScherm, -1, positie, afmeting, style=wx.TR_DEFAULT_STYLE|wx.TR_HIDE_ROOT|wx.HSCROLL|wx.VSCROLL)
        self.raadpleegScherm   = raadpleegScherm
        self.log               = raadpleegScherm.log
    ...
    def _initObject(self, item):
        if not self.GetPyData(item):
            ...
            BAGobject = getBAGobjectBijIdentificatie(self.GetItemText(item).partition(" ")[2])
            BAGobject.leesActueelVoorkomenUitDatabase()
            self.SetPyData(item, BAGobject)
            
            if BAGobject.objectType() == "OPR":
                self._initItem(item, "WPL", BAGobject.gerelateerdeWoonplaats.waarde())
            if BAGobject.objectType() == "NUM":
                if BAGobject.gerelateerdeWoonplaats.waarde() <> "":
                    self._initItem(item, "WPL", BAGobject.gerelateerdeWoonplaats.waarde())                
                self._initItem(item, "OPR", BAGobject.gerelateerdeOpenbareRuimte.waarde())                        
                ao = BAGobject.getAdresseerbaarObject()
                if ao:
                    self._initItem(item, ao.objectType(), ao.identificatie.waarde())
            if BAGobject.objectType() in ["LPL", "SPL", "VBO"]:
                self._initItem(item, "NUM", BAGobject.hoofdadres.waarde())
                for nevenadres in BAGobject.nevenadres.waarde():
                    self._initItem(item, "NUM", nevenadres)
            if BAGobject.objectType() == "VBO":
                for pand in BAGobject.gerelateerdPand.waarde():
                    self._initItem(item, "PND", pand)
            if BAGobject.objectType() == "PND":
                for vbo in BAGobject.getVerblijfsobjecten():
                    self._initItem(item, "VBO", vbo.identificatie.waarde())
    ...
    def voegToe(self, identificatie):
        BAGobject = getBAGobjectBijIdentificatie(identificatie)
        if BAGobject:
            return self._initItem(self.GetRootItem(), BAGobject.objectType(), BAGobject.identificatie.waarde())
        else:
            return None
...
class BAGView(rt.RichTextCtrl):
    # Constructor
    def __init__(self, raadpleegScherm, positie, afmeting):
        rt.RichTextCtrl.__init__(self, raadpleegScherm, -1, "", positie, afmeting, style=wx.TE_READONLY|wx.TE_MULTILINE)
        self.raadpleegScherm      = raadpleegScherm
        self.BAGobject            = None
        self.font                 = wx.Font(8, wx.FONTFAMILY_MODERN, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL)
        self.SetFont(self.font)
        self.toonLevenscyclusKnop = wx.Button(self, label="Toon levenscyclus", pos=(10, self.GetSize().GetHeight() - 30))
    ...
    def toonLevenscyclus(self, event):
        ...
        while v < len(levenscyclus):
            ...
            begindatum = "%s%s-%s%s-%s%s%s%s" %(datum[6],datum[7],datum[4],datum[5],datum[0],datum[1],datum[2],datum[3])
            datum = levenscyclus[v].einddatumTijdvakGeldigheid.waarde()
            if datum == "":
                einddatum = "..."
            else:
                einddatum = "%s%s-%s%s-%s%s%s%s" %(datum[6],datum[7],datum[4],datum[5],datum[0],datum[1],datum[2],datum[3])
            self.WriteText("-----------------------------------------------------------------\n")
            self.BeginBold()
            ...
            while a < len(levenscyclus[v].attributen):
                if levenscyclus[v].attributen[a].enkelvoudig():
                    ...
                    if laatsteActieve <> -1 and levenscyclus[v].aanduidingRecordInactief.waarde() == "N" and levenscyclus[v].attributen[a].waarde() <> levenscyclus[laatsteActieve].attributen[a].waarde():
...
class BAGKaart(wx.Panel):
    ...
    def cursorPositieY(self):
        return self.centerY + (((wx.GetMousePosition().y - self.GetScreenPosition().y - self.GetSize().GetHeight()) * -1) - self.GetSize().GetHeight()/2) * self.schaal
    ...
    def toonGeometrie(self, BAGobject):
        ...
        if self.gebruikHuidigeKaart:
            ...
            rechthoek = "POLYGON((%f %f 0.0, %f %f 0.0, %f %f 0.0, %f %f 0.0, %f %f 0.0))" %(lX,lY,rX,lY,rX,rY,lX,rY,lX,lY)
...
class BAGRaadpleeg(wx.Dialog):
    ...
    def __init__(self, parent):
        wx.Dialog.__init__(self, parent, -1, 'Raadpleeg BAG Extract+ database', size=(1000, 800))

        self.sluitKnop = wx.Button(self, label="Sluit", pos=(5,745))
        self.Bind(wx.EVT_BUTTON, self.sluit, self.sluitKnop)

        self.log = wx.TextCtrl(self, -1, "", pos=(90,740), size=(900,30), style=wx.TE_READONLY|wx.TE_MULTILINE)
        self.log.SetBackgroundColour(self.sluitKnop.GetBackgroundColour())
        ...
        self.boom  = BAGBoom( self, positie=(180,0), afmeting=(320,340))
        self.view  = BAGView( self, positie=(500,0), afmeting=(490,740))
